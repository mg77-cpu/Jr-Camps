...
const PillNav: React.FC<PillNavProps> = ({
  ...
  return (
    <div className="relative z-[1000] w-full md:w-auto">
      <nav
        className={`w-auto md:w-max flex items-center justify-between md:justify-start box-border px-4 md:px-0 ${className}`}
        aria-label="Primary"
        style={cssVars}
      >
        {showLogo && (
          isRouterLink(items?.[0]?.href) ? (
            ...
            >
              {typeof logo === 'string' ? (
                ...
                <div ref={logoImgRef as any} className="w-full h-full flex items-center justify-center p-1.5">
                  {logo}
                </div>
              )}
            </Link>
          ...
          >
            {items.filter(item => !item.hideOnDesktop).map((item, i) => {
              ...
              const basePillClasses =
                'relative overflow-hidden inline-flex items-center justify-center h-full no-underline rounded-full box-border font-medium text-[15px] leading-none tracking-tight whitespace-nowrap cursor-pointer px-0';
